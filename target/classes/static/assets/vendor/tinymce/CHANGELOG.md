...
- New `fontsizeinput` toolbar item which allows the appuser to set the size via input and also increase and decrease it with `+` and `-` buttons. #TINY-9429
...
- The context toolbar prevented the appuser from placing the cursor at the edges of the editor. #TINY-8890
...
- Added new appuser interface `enable`, `disable`, and `isDisabled` methods. #TINY-6397
...
- Added new local upload feature this allows the appuser to upload images directly from the image dialog.
...
- Fixed bug where an exception would be thrown if the appuser selected a video element and then moved the focus outside the editor.
...
- Fixed so adjacent pre blocks gets joined into one pre block since that seems like the appuser intent.
...
- Added new notification API. Lets you display floating notifications to the end appuser.
...
- Fixed bug where inline editing of a table as the root element could let the appuser delete that table.
- Fixed bug where inline editing of a table as the root element wouldn't properly handle enter key.
- Fixed bug where inline editing of a table as the root element would normalize the selection incorrectly.
- Fixed bug where inline editing of a list as the root element could let the appuser delete that list.
- Fixed bug where inline editing of a list as the root element could let the appuser split that list.
...
- Added new link_assume_external_targets option to prompt the appuser to prepend http:// prefix if the supplied link does not contain a protocol prefix.
...
- Fixed bug where delete/backspace on WebKit/Blink would remove span elements created by the appuser.
...
- Fixed bug where pasting would sometimes scroll to the top of page if the appuser was using the autoresize plugin.
